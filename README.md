...
| `ore_grade_source` | Builds the ORE grading scaffold for a source: five dimensions (three core, two declared extensions), what must be recorded on each, the named gaps an account leaves open, opacity and monitoring obligations | Before admitting any source into a system that decides from it |
...
| `ore_audit_finding` | Audits a finding against the five obligations of STRUCK, with observed evidence, gaps, fixes and systemic patterns | Before publishing a finding, or on someone else's |
